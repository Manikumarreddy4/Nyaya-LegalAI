...
export function generateHtmlReport(results, baseUrl = 'http://localhost:5173') {
  ...
  // Group calculations
  const fTotal = results.filter(r => r.id.startsWith('WEB-F')).length;
  const fPassed = results.filter(r => (r.id.startsWith('WEB-F') && (r.status === 'PASS' || r.status === 'Passed'))).length;
  
  const uiTotal = results.filter(r => r.id.startsWith('WEB-U')).length;
  const uiPassed = results.filter(r => (r.id.startsWith('WEB-U') && (r.status === 'PASS' || r.status === 'Passed'))).length;

  const sTotal = results.filter(r => r.id.startsWith('WEB-S')).length;
  const sPassed = results.filter(r => (r.id.startsWith('WEB-S') && (r.status === 'PASS' || r.status === 'Passed'))).length;
...
<head>
  ...
  <title>Web Selenium E2E Test Report</title>
  <style>
    :root {
      --bg-dark: #0f172a;
      --card-bg: #1e293b;
    ...
    .badge-pass { background-color: rgba(99, 102, 241, 0.15); color: #818cf8; }
...
<body>
  <div class="container">
    <header>
      <div>
        <h1>Web Selenium E2E Report</h1>
        <p style="margin:4px 0 0 0; color:var(--text-muted); font-size:14px;">React Web Application E2E execution summary</p>
      </div>
      <div class="meta-info">
        <div>Date: ${new Date().toLocaleDateString()}</div>
        <div>Time: ${new Date().toLocaleTimeString()}</div>
        <div>Target Env: ${baseUrl}</div>
      </div>
    </header>
