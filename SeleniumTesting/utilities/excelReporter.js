...
export async function generateExcelReport(results, baseUrl = 'http://localhost:5173', browserName = 'chrome') {
  ...
  summarySheet.getRow(1).values = [
    'Execution Date', 'Environment', 'Total Tests', 'Passed', 'Failed', 'Skipped', 'Pass Percentage', 'Execution Duration'
  ];
  ...
  testCasesSheet.getRow(1).values = [
    'Test ID', 'Module', 'Scenario Name', 'Browser', 'Status', 'Start Time', 'End Time', 'Duration'
  ];
  ...
  results.forEach((r, idx) => {
    const rowNum = 2 + idx;
    const startTimeStr = new Date(Date.now() - r.duration).toLocaleTimeString();
  ...
  failedSheet.getRow(1).values = [
    'Test Name', 'Failure Reason', 'Screenshot Path', 'Browser', 'URL'
  ];
  ...
  logsSheet.getRow(1).values = [
    'Timestamp', 'Test Name', 'Step Description', 'Result', 'Remarks'
  ];
  ...
  results.forEach(r => {
    ...
    logsSheet.getRow(logRowIdx).values = [
      timeStr,
      r.name,
      'Initialize and run Selenium web test scenarios checks',
      r.status,
  ...
  const mainPath = path.join(reportsDir, 'E2E_Report.xlsx');
  ...
  const historyPath = path.join(historyDir, 'E2E_Report.xlsx');
